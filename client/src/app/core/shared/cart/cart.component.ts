...
export class CartComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.cartService
      ...
      .subscribe((res) => {
        this.cart = res.data;
        this.cartForm = this.toFormGroup(this.cart.plants);
        this.onChanges();
      });
  }
  ...
  toFormGroup(plants: Plant[]): FormGroup {
    const group: any = {};

    plants.forEach(plant => {
      group[plant._id] = new FormControl(
        plant.qty || '', [
          Validators.required,
          Validators.min(1),
          Validators.max(20)
  ...
  onRemove(): void {
    this.cartService
      ...
      .subscribe(() => {
        ...
        this.cart.plants = this.cart.plants.filter(b => b._id !== this.lastDeleteId);
  ...
  reCalcSum(formValues: object): void {
    let price = 0;
    for (const b of this.cart.plants) {
